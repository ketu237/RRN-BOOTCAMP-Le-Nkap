...
<div className="bg-white-900">
  ...
  <div className="min-h-screen p-5 flex items-center justify-center bg-white-600">
    <div className="w-full md:max-w-xl shadow-lg rounded-xl p-8 flex flex-col bg-white">
      <h2 className="font-bold text-2xl md:text-3xl text-center uppercase mb-6">Dashboard</h2>
      ...
      {!transactionsLoading && transactions.length > 0 && (
        <div className="my-8">
          ...
          <div className="flex justify-center mb-4">
            {showWarning && <p className='text-red-500 font-bold'></p>}
            {showEncouragement && <p style={{ color: 'green' }} className='font-bold'></p>}
          </div>
          {/* Button to Add New Transaction */}
          <div className="flex justify-center mb-4">
            <Link to="/transactions" className='rounded-xl px-4 py-1 text-sm text-red-600 font-semibold border border-red-200 hover:text-white hover:bg-red-600 hover:border-transparent hover:scale-105 duration-300 focus:outline-none focus:ring-2 focus:ring-red-600 focus:ring-offset-2'>
              Add New Transaction +
            </Link>
          </div>
          {/* Pie Chart */}
          <div className="flex justify-center">
            <Chart options={{ labels: ['Income', 'Expense'], colors: ['#008FFB', '#FF4560'] }} series={[incomeTotal, expenseTotal]} type="pie" width="380" />
            <div className="grid ml-4 mt-4 gap-2">
      ...
      {shouldDisplayGraphs && (
        <div className="my-8">
          <Chart options={graphData.options} series={graphData.series} type="area" height={350} />
        </div>
      )}
      ...
      <div className="my-8">
        {categoriesLoading ? (
          <div className="flex justify-center">
            <FontAwesomeIcon icon={faSpinner} spin className="text-red-600 text-4xl" />
          </div>
        ) : (
          <div className="flex flex-wrap">
            ...
            {categories.map(category => (
              <div key={category.id} className="w-full md:w-1/2 lg:w-1/3 xl:w-1/4 p-2">
                <div className="bg-white rounded-xl shadow-md px-2 mt-4 flex mx-auto hover:bg-red-950">
                  <div className="mx-auto">
                    <p className="font-semibold text-red-700 ">Name: {category.name}</p>
            ...
            <div className="flex justify-center mt-4">
              <Link to="/categories" className='rounded-xl px-4 py-1 text-sm text-red-600 font-semibold border border-dotted border-red-200 hover:text-white hover:bg-red-600 hover:border-transparent hover:scale-105 duration-300 focus:outline-none focus:ring-2 focus:ring-red-600 focus:ring-offset-2'>
                Add New Category +
              </Link>
            </div>
          </div>
