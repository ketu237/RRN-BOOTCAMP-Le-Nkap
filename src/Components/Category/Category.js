...
const CategoryForm = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white-900">
    <Navbar />
    {shouldDisplayMessage && <p className='font-semibold text-sm text-red-500'>All good buddy</p>}
    <div className="flex-1 p-5 flex items-center justify-center bg-white-500">
      <div className="w-full md:max-w-md shadow-lg rounded-xl p-8 flex flex-col bg-white">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="flex flex-col">
            ...
            <div className="flex justify-center">
              ...
              {loading ? (
                <FontAwesomeIcon icon={faSpinner} spin size="lg" className="text-white-600 mr-2" />
              ) : (
                <button
                  className="mt-4 rounded-xl px-4 py-1 text-sm text-white-600 font-semibold border border-green-200 hover:text-white hover:bg-red-600 hover:border-transparent hover:scale-105 duration-300 focus:outline-none focus:ring-2 focus:ring-red-600 focus:ring-offset-2"
                  type="submit"
                >
                  Add Category
                </button>
  ...
  <div className="overflow-x-auto">
    <table className="table-auto border-collapse border w-full">
      <thead>
        <tr>
          <th className="border px-4 py-2 text-center sm:px-6 md:px-8">Name</th>
          <th className="border px-4 py-2 text-center sm:px-6 md:px-8">Type</th>
        </tr>
      </thead>
      <tbody>
        {expenseCategories.map((category, index) => (
          <tr key={index}>
            <td className="border px-4 py-2 text-center sm:px-6 md:px-8">{category.name}</td>
            <td className="border px-4 py-2 text-center sm:px-6 md:px-8">{category.type}</td>
          </tr>
        ))}
      </tbody>
    </table>
